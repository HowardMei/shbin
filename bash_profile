...
export PATH=/bin:/sbin:/usr/bin:/usr/sbin:/usr/local/bin:/usr/local/sbin:/opt/bin:/opt/sbin:/opt/xbin
[ -d "$HOME/.baseinit/shbin" ] && export PATH="$HOME/.baseinit/shbin:$PATH"
...
alias pstop='__psls() { local p="$1"; watch -n 0.5 "ps -e -o pid,uname,comm,cmd,pcpu,pmem --sort=-pcpu,-pmem | head -n ${p:=11}"; unset -f __psls; }; __psls'
...
alias vex='__vex() { [ $# -eq 1 ] && [ "$1" == "-q" ] && [ -r "bin/activate" ] && deactivate && return 0;[ -r "${!#}/bin/activate" ] && cd "${!#}" && source bin/activate || virtualenv "$@";[ -d "${!#}" ] && cd "${!#}";[ -r "bin/activate" ] && source bin/activate; unset -f __vex; }; __vex'
...
# Personalize the shinit bash environment
[ -r "$HOME/.bash_personal" ] && . "$HOME/.bash_personal"
